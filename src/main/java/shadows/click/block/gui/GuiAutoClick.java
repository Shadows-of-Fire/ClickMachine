...
public class GuiAutoClick extends ContainerScreen<ContainerAutoClick> {
	...
	protected void renderLabels(MatrixStack stack, int mouseX, int mouseY) {
		...
		this.font.draw(stack, player.inventory.getDisplayName().getString(), 8, this.imageHeight - 96 + 2, 4210752);
	}
	...
	protected void renderTooltip(MatrixStack stack, int x, int y) {
		...
		if (isHovering(150, 26, 21, 64, x, y)) {
			if (ClickMachineConfig.usesRF) {
				...
				comps.add(new TranslationTextComponent("gui.clickmachine.power", menu.data.get(0), ClickMachineConfig.maxPowerStorage));
				comps.add(new TranslationTextComponent("gui.clickmachine.power.usage", ClickMachineConfig.powerPerSpeed[menu.data.get(1)]));
				GuiUtils.drawHoveringText(stack, comps, x, y, width, height, 0xFFFFFF, font);
			} else {
				List<ITextComponent> comps = new ArrayList<>(1);
				comps.add(new TranslationTextComponent("gui.clickmachine.rainbow_magic"));
				GuiUtils.drawHoveringText(stack, comps, x, y, width, height, 0xFFFFFF, font);
			}
		}
	}
	...
	public static void colorBlit(MatrixStack pMatrixStack, int pX, int pY, float pUOffset, float pVOffset, int pUWidth, int pVHeight, int color) {
		innerBlit(pMatrixStack, pX, pX + pUWidth, pY, pY + pVHeight, 0, pUWidth, pVHeight, pUOffset, pVOffset, 256, 256, color);
	}

	private static void innerBlit(MatrixStack pMatrixStack, int pX1, int pX2, int pY1, int pY2, int pBlitOffset, int pUWidth, int pVHeight, float pUOffset, float pVOffset, int pTextureWidth, int pTextureHeight, int color) {
